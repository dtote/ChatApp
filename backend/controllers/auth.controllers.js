...
export const signupFacial = async (req, res) => {
  try {
    const { username, password, email, faceDescriptor } = req.body;
    ...
    const axiosRetry = async (url, data, retries = 3) => {
      for (let i = 0; i < retries; i++) {
        try {
          ...
          // Si es rate limiting, esperar más tiempo con backoff exponencial
    ...
    let keys, dsaKeys;
    try {
      const keysResponse = await axiosRetry(`${ENV_CONFIG.PQCLEAN_API_URL}/generate_keys`, { kem_name: "ML-KEM-512" });
    ...
    try {
      const dsaKeysResponse = await axiosRetry(`${ENV_CONFIG.PQCLEAN_API_URL}/generate_ml_dsa_keys`, { ml_dsa_variant: "ML-DSA-44" });
      dsaKeys = dsaKeysResponse.data;
    } catch (error) {
      if (error.response?.status === 429) {
        return res.status(429).json({ error: "Too many requests. Please wait a moment and try again." });
      }
    ...
    console.error("Error in signupFacial:", error);
    ...
    // Si es un error de duplicado (username o email ya existe)
    if (error.code === 11000) {
...
export const signup = async (req, res) => {
  try {
    ...
    const profilePicFile = req.file; // Uploaded file
    ...
    // Default profile picture path if none provided
    let profilePic = `https://api.dicebear.com/7.x/adventurer/svg?seed=${encodeURIComponent(username)}`;

    // If an image was sent, update the path
    if (profilePicFile) {
      profilePic = `/uploads/${profilePicFile.filename}`; // Relative path
    }
...
export const loginFacial = async (req, res) => {
  try {
    ...
    // If there are no valid descriptors, stop the process

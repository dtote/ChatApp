...
// 3. Obtener los mensajes de la comunidad, desencriptarlos y verificarlos
    ...
    const decryptedMessages = await Promise.all(community.messages.map(async (msg) => {
      try {
        ...
        if (msg.signature && msg.publicKeyDSA) {
          try {
            const verifyResponse = await axios.post('http://127.0.0.1:5001/verify', {
...
router.post('/:id/messages', upload.none(), protectRoute, async (req, res) => {
  ...
  try {
    ...
    const receiverSocketId = getReceiverSocketId(communityId);
    if (receiverSocketId) {
